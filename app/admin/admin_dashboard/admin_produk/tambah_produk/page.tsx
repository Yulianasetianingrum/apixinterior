...
function ImagePickerModal({
  ...
  const [cacheNonce, setCacheNonce] = useState<number>(0);
        ...
        {loading ? (
          ...
          >
            {filtered.map((g) => {
              ...
              const baseSrc = g.thumbUrl || g.url;
              const modalImgSrc = `${baseSrc}${baseSrc.includes("?") ? "&" : "?"}cb=picker_${g.id}_${cacheNonce}`;
              return (
                <button
                  key={g.id}
                  type="button"
...
const VariasiKombinasiWidget = memo(function VariasiKombinasiWidget({
  ...
  useEffect(() => {
    ...
    function render() {
      try {
        if (!stepsEl || !pageEl) {
          console.warn("VCombo: render targets missing, skipping...");
          return;
        }
        ...
        console.error("VCombo render error", err);
        notify("Error saat render Variasi: " + (err as any).message);
      }
    }
  ...
  `;
    ...
    try {
      ...
      console.error("VCombo bootstrap error", e);
    }
